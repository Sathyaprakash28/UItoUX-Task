...
export class RegisterComponent implements OnInit {
  ...
  RegisterCustomer(regForm:any){
    if(this.model.product.length<10 || this.model.Password!=this.model.ConfirmPassword ){
    if(this.model.product.length<10){
      this.productalert="Invalid Number, Enter Valid product Number"
    }
