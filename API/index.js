...
sqlquery="SELECT COMPANY.COMPANY_NAME,PRODUCT.PRODUCT_ID,PRODUCT.PRODUCT_NAME,PRODUCT.PRODUCT_IMAGE,PRODUCT.PRICE FROM PRODUCT INNER JOIN COMPANY ON COMPANY.COMPANY_ID=PRODUCT.COMPANY_ID WHERE PRODUCT_ID="+req.params.ProductId;
...
app.get('/getAllProduct',(req,res)=>{
    sqlquery="SELECT CONCAT(COMPANY.COMPANY_NAME,' ',PRODUCT.PRODUCT_NAME) AS PRODUCT_NAME,PRODUCT.PRODUCT_ID,PRODUCT.PRODUCT_IMAGE,PRODUCT.PRICE FROM PRODUCT INNER JOIN COMPANY ON COMPANY.COMPANY_ID=PRODUCT.COMPANY_ID"
connection.query(sqlquery,(err,result)=>{
    if(err) console.log(err)
    if(result.length){
        console.log(result);
...
sqlgetData1="SELECT COMPANY.COMPANY_NAME,PRODUCT.PRODUCT_ID,PRODUCT.PRODUCT_NAME,PRODUCT.PRODUCT_IMAGE,PRODUCT.PRICE FROM PRODUCT INNER JOIN COMPANY ON COMPANY.COMPANY_ID=PRODUCT.COMPANY_ID ORDER BY PRODUCT.PRODUCT_ID LIMIT "+req.params.limit;
...
app.post('/processRegister',(req,res)=>{
    ...
    sqlcheck="SELECT * FROM CUSTOMER WHERE EMAIL='"+email+"' OR product='"+product+"'"
    connection.query(sqlcheck,(err,result)=>{
        if(err) console.log(err)
        if(result.length>0) res.send({success:false})
        else{
            sqlquery="INSERT IGNORE INTO CUSTOMER(Name,Email,product,Password) VALUES('"+name+"','"+email+"','"+product+"','"+password+"')"
    connection.query(sqlquery,(err,result)=>{
        if(err) console.log(err)
        
        res.send({success:true})
        var message=' '+name+' Welcome to UItoUx you have been successfully registered with Email '+email+''
        sendMessage(message,product)
        sendEmail(email,'UItoUX New Customer','<h4>'+message+'</h4>')
    })
        }
...
app.post('/processLogin',(req,res)=>{
    console.log(req.body)
    sqlquery="Select CUSTOMER_ID,NAME,EMAIL,product,ADDRESS,PINCODE,LOCALITY,CITY,STATE,ADDRESS_TYPE from customer where email='"+req.body.email+"' and password='"+req.body.password+"' "
    connection.query(sqlquery,(err,result)=>{
...
app.post('/changePassword',(req,res)=>{
    ...
    connection.query(sqlquery,(err,result)=>{
        ...
        if (result.affectedRows>0) {
            res.send({success:true})
            var message='<h2>Hello User,<br/>Your Password Has Been Sucessfully Changed <br/> Your New Password is '+req.body.Password+' </h2> <br/> <h3> You Can Login to UItoUx from <a href="http://localhost:4200/">Here</a></h2>'
            sendEmail(req.body.email,'UItoUx Password Changed',message)
        }
        else res.send({success:false})
    })
})
...
function sendMessage(message,product){
...
var order="Order Placed: Your Order for "+products+" with OrderID "+order_id+" Amounting to Rs. "+total+" has been Recieved . Thankyou for Shopping with UItoUx."
var message='<h4> Regards '+req.body.customer_name+'</h4><br/><h4>'+order+'</h4>'
console.log(order)

var product=req.body.product
console.log(product)
sendMessage(order,product)
sendEmail(req.body.email,'UItoUx Order Placed',message)
...
app.post('/sendEmail',(req,res)=>{
    ...
    connection.query(sqlquery,(err,result)=>{
        if(err) console.log(err)
        if(result.length>0){
            var message='<h2>Hello '+result[0].NAME+',<br/>Your Password is '+result[0].PASSWORD+' <br/>  You Can Login to UItoUx from <a href="http://localhost:4200/">Here</a></h2><br/><h3> You can Change Your Password from <a href="http://localhost:4200/changePassword?email='+email+'">Here</a></h3>'
            sendEmail(email,'UItoUx Forget Password',message)
            res.send({success:true})
        }
        else res.send({success:false})
    })
...
app.post('/getAdminOrders',(req,res)=>{
    if(req.body.id==1){
        sqlquery="select ORDERS.ORDER_ID,ORDERS.CUSTOMER_NAME,ORDERS.PRODUCT_PURCHASED,ORDERS.PRODUCT_PRICE,ORDERS.PAYMENT_MODE,ORDERS.PURCHASE_DATE,ORDERS.PURCHASE_TIME,CONCAT(CUSTOMER.ADDRESS,' ',CUSTOMER.PINCODE,' ',CUSTOMER.CITY,' ',CUSTOMER.STATE) AS ADDRESS ,CUSTOMER.product,CUSTOMER.EMAIL FROM ORDERS INNER JOIN CUSTOMER ON ORDERS.CUSTOMER_ID=CUSTOMER.CUSTOMER_ID"
    }
    else{
...
available=Boolean(available);
    ...
    sqlproduct="INSERT INTO PRODUCT(COMPANY_ID,PRODUCT_NAME,PRODUCT_IMAGE,PRICE) VALUES((SELECT COMPANY.COMPANY_ID FROM COMPANY WHERE COMPANY_NAME='"+company+"'),'"+product+"','"+image+"','"+range+"')"
...
app.get('/UItoUX',(req,res)=>{
    res.send("Welcome to the Server of UItoUx E-Commerce Website")
})
